...
HRESULT ProxySite::LoadPDM(_In_ DWORD attachType, _Out_ CComPtr<IRemoteDebugApplication>& spRemoteDebugApplication)
{
	...
	if (spResult.vt == VT_UINT_PTR)
	{
		// Since VariantClear does not support VT_UINT_PTR which IDM_DEBUGGERDYNAMICATTACH returns, we should just detach the smart pointer.
		VARIANT releasedVar;
		::VariantInit(&releasedVar);
		hr = spResult.Detach(&releasedVar);
		ATLASSERT(hr == S_OK); hr;
	}
